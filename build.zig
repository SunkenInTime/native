...
pub fn build(b: *std.Build) void {
    ...
    addFileContainsCheckStep(b, file_contains_checker, test_step, "test-macos-shared-renderer-client", "Verify the device-less widget client keeps the shared-renderer contract", &.{
        ...
        .{ .path = "src/platform/macos/appkit_host.m", .pattern = "will reconnect" },
    ...
    addFileContainsCheckStep(b, file_contains_checker, test_step, "test-appkit-production-metal-lifetime", "Verify AppKit ships the measured process-lifetime Metal architecture", &.{
        ...
        .{ .path = "src/platform/macos/appkit_host.m", .pattern = "gpuSurfaceTextureForImageKey" },
        .{ .path = "src/platform/macos/appkit_host.m", .pattern = "op->type = 6" },
